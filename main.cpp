...
void editBook(Book library[], int count)
{
    ...
    while (true)
    {
        ...
        if (cin >> targetId)
        {
            clearInput(); // ✅ Очищаємо **один раз** після успішного вводу ID
            break;
        }
        else
        {
            cout << "Помилка: ID має бути цілим числом. Спробуйте ще раз.\n";
            clearInput();
        }
    }

    // 🔍 Пошук індексу книги (замість bool + break в циклі)
    int bookIndex = -1;
    for (int i = 0; i < count; i++)
    {
    ...
    if (bookIndex == -1)
    {
        ...
        return; // ✅ Вихід раніше — чистіше і без зайвого "if (!found)"
    }

    // 📖 Показуємо знайдену книгу
    ...
    getline(cin, newTitle); // ✅ працює коректно, бо буфер уже очищено

    string newAuthor;
    cout << "Введіть нового автора (або Enter, щоб залишити без змін): ";
    getline(cin, newAuthor);

    // ✏️ Оновлення лише якщо введено щось
    if (!newTitle.empty()) library[bookIndex].title = newTitle;
    if (!newAuthor.empty()) library[bookIndex].author = newAuthor;
